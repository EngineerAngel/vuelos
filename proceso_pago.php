...
if (!$clave || empty($nombre) || empty($num_tarjeta)) {
    redirigir_con_error('pago.php', 'Todos los campos son requeridos');
}
if (!validar_tarjeta($num_tarjeta)) {
    redirigir_con_error('pago.php', 'El número de tarjeta debe tener 16 dígitos');
}
if (!empty($fecha_vencimiento) && !validar_fecha_vencimiento($fecha_vencimiento)) {
    redirigir_con_error('pago.php', 'Fecha de vencimiento inválida o expirada (formato MM/AAAA)');
}
...
$stmt = $pdo->prepare(
    'UPDATE reservas SET estado_pago="pagado" WHERE clave_reserva=? AND id_usuario=?'
);
